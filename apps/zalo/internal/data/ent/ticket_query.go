...
type TicketQuery struct {
	...
	withTicketSeats      *TicketSeatQuery
	modifiers            []func(*sql.Selector)
	withNamedTicketSeats map[string]*TicketSeatQuery
...
// QueryTicketSeats chains the current query on the "ticketSeats" edge.
func (_q *TicketQuery) QueryTicketSeats() *TicketSeatQuery {
	query := (&TicketSeatClient{config: _q.config}).Query()
...
func (_q *TicketQuery) Clone() *TicketQuery {
	...
	return &TicketQuery{
		...
		withTicketSeats: _q.withTicketSeats.Clone(),
...
// WithTicketSeats tells the query-builder to eager-load the nodes that are connected to
// the "ticketSeats" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *TicketQuery) WithTicketSeats(opts ...func(*TicketSeatQuery)) *TicketQuery {
	query := (&TicketSeatClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withTicketSeats = query
	return _q
}
...
func (_q *TicketQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Ticket, error) {
	...
	_spec.Node.Schema = _q.schemaConfig.Ticket
	...
	if query := _q.withTicketSeats; query != nil {
		if err := _q.loadTicketSeats(ctx, query, nodes,
			func(n *Ticket) { n.Edges.TicketSeats = []*TicketSeat{} },
			func(n *Ticket, e *TicketSeat) {
				n.Edges.TicketSeats = append(n.Edges.TicketSeats, e)
	...
	for name, query := range _q.withNamedTicketSeats {
		if err := _q.loadTicketSeats(ctx, query, nodes,
			func(n *Ticket) { n.appendNamedTicketSeats(name) },
			func(n *Ticket, e *TicketSeat) {
				n.appendNamedTicketSeats(name, e)
				if !e.Edges.loadedTypes[0] {
					e.Edges.Ticket = n
				}
			}); err != nil {
...
func (_q *TicketQuery) loadTicketSeats(ctx context.Context, query *TicketSeatQuery, nodes []*Ticket, init func(*Ticket), assign func(*Ticket, *TicketSeat)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(ticketseat.FieldTicketID)
	}
	query.Where(predicate.TicketSeat(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(ticket.TicketSeatsColumn), fks...))
	}))
...
func (_q *TicketQuery) sqlQuery(ctx context.Context) *sql.Selector {
	...
	t1.Schema(_q.schemaConfig.Ticket)
	ctx = internal.NewSchemaConfigContext(ctx, _q.schemaConfig)
...
func (_q *TicketQuery) Modify(modifiers ...func(s *sql.Selector)) *TicketSelect {
	_q.modifiers = append(_q.modifiers, modifiers...)
	return _q.Select()
}

// WithNamedTicketSeats tells the query-builder to eager-load the nodes that are connected to the "ticketSeats"
// edge with the given name. The optional arguments are used to configure the query builder of the edge.
func (_q *TicketQuery) WithNamedTicketSeats(name string, opts ...func(*TicketSeatQuery)) *TicketQuery {
	query := (&TicketSeatClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	if _q.withNamedTicketSeats == nil {
		_q.withNamedTicketSeats = make(map[string]*TicketSeatQuery)
	}
	_q.withNamedTicketSeats[name] = query
	return _q
}
...
// Modify adds a query modifier for attaching custom logic to queries.
func (_s *TicketSelect) Modify(modifiers ...func(s *sql.Selector)) *TicketSelect {
